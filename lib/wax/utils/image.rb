...
module Wax
  module Utils
    module Image
      ...
      def self.new_variant(image, size, _max = DEFAULT_MAX)
      ...
      def self.full_variant(image, max)
        return image.dup.thumbnail_image image.width, height: 1_000_000 if image.width < max

        image.dup.thumbnail_image max, height: 1_000_000
      end
